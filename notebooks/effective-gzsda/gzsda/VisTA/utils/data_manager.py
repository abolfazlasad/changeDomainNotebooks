...
def build_data_loader(
    ...
    if increments is not None:
        filtered_indices = [
            i for i in range(len(data_source))
            if data_source[i].label in increments
        ]
        data_source = torch.utils.data.Subset(data_source, filtered_indices)
    ...
    def new_task(self, task = None):
        ...
        if self.dataset.train_u:
            ...
            train_loader_u = build_data_loader(
                ...
                data_source=self.dataset.train_u,
                ...
                increments=[i for i in range(self.cfg.TRAIN.PER * task, self.cfg.TRAIN.PER * task + self.cfg.TRAIN.PER)]
            )

        step_loader = None
        step_loader = build_data_loader(
            self.cfg,
        ...
        s1_loader = build_data_loader(
            ...
            is_train=False,
            dataset_wrapper=self.dataset_wrapper,
            increments=[i for i in range(self.cfg.TRAIN.PER)]
        )

        final_loader = build_data_loader(
            self.cfg,
            sampler_type=self.cfg.DATALOADER.TEST.SAMPLER,
            ...
            is_train=False,
            dataset_wrapper=self.dataset_wrapper,
            increments=[i for i in range(0, self.cfg.TRAIN.PER * task + self.cfg.TRAIN.PER)]
        )

        return train_loader_u, step_loader, s1_loader, final_loader
